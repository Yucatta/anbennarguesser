...
function loadAllDataOnce(): AppData {
  try {
    const root = process.cwd() + "/public/anbennar";

    const emptylands = JSON.parse(
      fs.readFileSync(path.join(root, "emptylands.json"), "utf-8")
    );

    const pathsJson = JSON.parse(
      fs.readFileSync(path.join(root, "provincesoutlines.json"), "utf-8")
    );
    const Countries: Array<[string, string, string, number[], number]> =
      JSON.parse(
        fs.readFileSync(path.join(root, "anbennarcountries.json"), "utf-8")
      );
